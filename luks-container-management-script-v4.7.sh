...
TMP_MAPPER_NAME="luks_tmp_$(openssl rand -hex 4 2>/dev/null || tr -dc 'a-z0-9' < /dev/urandom | head -c 8)"
...
cleanup() {
    ...
    if [[ $exit_code -eq "0" ]]; then
        # Cleanup for container creation
        if [[ -n "${CON_ALIAS}" ]]; then
...
check_disk_space() {
    ...
    local required_mb="$2"
    local available_mb
    available_mb=$(df --output=avail -B 1M "${target_dir}" | tail -n 1)
...
find_con () {
    ...
    echo
    read -rp "Specify LUKS container directory (full path): " exs_path
...
verify_and_mount_container() {
    ...
    if ! findmnt "/dev/mapper/${TMP_MAPPER_NAME}" &>/dev/null; then
        ...
        local loop_dev
        if ! loop_dev=$(sudo losetup -f --show --direct-io=on "${container_path}" 2>/dev/null) || [[ -z "$loop_dev" ]]; then
        ...
        if ! sudo cryptsetup luksOpen "$loop_dev" "${TMP_MAPPER_NAME}"; then
    ...
    if ! sudo mount "/dev/mapper/${TMP_MAPPER_NAME}" "${TMP_MOUNT}"; then
...
backup_luks_header() {
    ...
    read -rp "Enter backup directory (full path): " backup_path
    ...
    local backup_file
    backup_file="$backup_path/$(basename "${container_path}")_header_$(date +%Y%m%d).img"

    echo -e "\n${CYAN}Backing up to:${NC} $backup_file"
    if sudo cryptsetup luksHeaderBackup "${container_path}" --header-backup-file "$backup_file"; then
...
restore_luks_header() {
    ...
    read -rp "Enter path to header backup file (eg. /home/user/luksheader.img): " backup_file
    ...
    if [[ "$backup_size" -lt "1048576" ]]; then  # Less than 1MB is suspicious
        ...
        read -rp "Continue anyway? [y/N]: " continue_small
    ...
    read -rp "This helps ensure you're restoring the correct header [y/N]: " check_uuid
    check_uuid=${check_uuid:-n}
    ...
    if sudo cryptsetup luksHeaderRestore "${container_path}" --header-backup-file "$backup_file"; then
        echo -e "\n${GREEN}══════════════════════════════════════════════════════${NC}"
...
erase_luks_header() {
    ...
    read -rp "Create backup before erasing? [Y/n]: " create_backup
    ...
    if sudo dd if=/dev/urandom of="${container_path}" bs=1M count="$header_size_mb" conv=notrunc status=progress; then
...
add_keyfile() {
    ...
    read -rp "Enter choice [1-2]: " keyfile_choice

    local keyfile_path

    if [[ "$keyfile_choice" == "1" ]]; then
        read -rp "Enter directory to save keyfile (full path): " keyfile_dir
        ...
        read -rp "Enter keyfile name (e.g. my_key.key): " keyfile_name
        ...
        while true; do
            read -rp "Keyfile size in bytes (recommended: 4096): " keyfile_size
            ...
            if [[ "$keyfile_size" -gt "65536" ]]; then
                ...
                read -rp "Continue with $keyfile_size bytes? [y/N]: " confirm_large
        ...
        read -rp "Enter path to existing keyfile: " keyfile_path
    ...
    if sudo cryptsetup luksAddKey "${container_path}" "${keyfile_path}"; then
        echo -e "\n${GREEN}══════════════════════════════════════════════════════${NC}"
...
change_password() {
    ...
    case "$pwd_choice" in
        1)
            ...
            if sudo cryptsetup luksAddKey "${container_path}"; then
        ...
        2)
            ...
            if sudo cryptsetup luksChangeKey "${container_path}"; then
...
remove_keyslot() {
    ...
    echo
    echo -e "${YELLOW}Remaining keyslot cannot be a keyfile (sanity check).${NC}"
    echo
    ...
    while true; do
        ...
        read -rp "Type 'YES' in uppercase to confirm: " confirm
        if [[ "$confirm" != "YES" ]]; then
            echo -e "${BLUE}Operation cancelled.${NC}"
            return 0
        fi
    ...
    echo -e "${YELLOW}You will need to enter a remaining valid passphrase${NC}"
    echo -e "${YELLOW}(remaining keyslot cannot be a keyfile)${NC}"

    if sudo cryptsetup luksKillSlot "${container_path}" "$slot_number"; then
        echo -e "\n${GREEN}══════════════════════════════════════════════════════${NC}"
...
show_container_info() {
    ...
    if [[ -n "$json_dump" ]] && command -v jq &>/dev/null; then
        ...
        while IFS= read -r line; do
            ...
            # Check if we're leaving the Keyslots section (next main section that does not start with whitespace)
...
while true; do
    ...
    echo -e "  ${BLUE}8)${NC} ${CYAN}Add keyslot/Change password${NC}"
    ...
    if [[ "$TASK_NUM" == "11" ]]; then
        ...
        CALC_CONT_SIZE_BYTES=$(wc -c < "$CONT_TARGET1")
        ...
        if [[ "$TASK_NUM" == "2" ]]; then
            ...
            read -rp "Is this correct? [y/n] " RESP1
            ...
            if [[ "${RESP1,,}" == "y" ]]; then
                ...
                if ! loop_dev=$(sudo losetup -f --show --direct-io=on "$CONT_TARGET1" 2>/dev/null) || [[ -z "$loop_dev" ]]; then
                ...
                if ! sudo cryptsetup resize "/dev/mapper/${TMP_MAPPER_NAME}"; then
            ...
            if [[ "${RESP2,,}" == "y" ]]; then
                ...
                if [[ $resize_result -ne "0" ]]; then
                    ...
                    if ! sudo resize2fs -M "/dev/mapper/${TMP_MAPPER_NAME}"; then
                ...
                if ! sudo cryptsetup resize --size "${payload_sectors}" "/dev/mapper/${TMP_MAPPER_NAME}"; then
                ...
                if ! sudo truncate -s "${container_mb}M" "$CONT_TARGET1"; then
                ...
                if  verify_and_mount_container "$CONT_TARGET1" "1"; then
                    echo -e "\n${GREEN}══════════════════════════════════════════════════════${NC}"
                    echo -e "${GREEN}✓ Container successfully shrunk!${NC}"
                    echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"

                    final_cont_bytes=$(wc -c < "$CONT_TARGET1")
...
while true; do
    ...
    echo -e "${BLUE}===============================================${NC}"
    read -rp "Create a name for your LUKS container: " CON_NAME
...
while true;do
    ...
    read -rp "(eg. company-records or cloud_archive): " CON_ALIAS
...
while true; do
    ...
    echo -e "${BLUE}===============================================${NC}"
    read -rp "Type the LUKS container storage path (full path): " SOURCED
...
if [[ "$keyfile_setup_choice" == "2" ]] || [[ "$keyfile_setup_choice" == "3" ]]; then
    ...
    read -rp "Enter directory to save keyfile: " keyfile_dir
    ...
    read -rp "Enter keyfile name (e.g., ${CON_NAME}.keyfile): " keyfile_name
    ...
    while true; do
        read -rp "Keyfile size in bytes (default: 4096): " keyfile_size
        keyfile_size=${keyfile_size:-4096}
...
if ! LOSP=$(sudo losetup -f --show --direct-io=on "$MYPATH/$CON_NAME.bin" 2>/dev/null) || [[ -z "$LOSP" ]]; then
    echo -e "${RED}Failed to get loop device.${NC}\n"
    exit 1
fi
...
if [[ "$USE_KEYFILE" == true ]] && [[ "$keyfile_setup_choice" == "3" ]]; then
    ...
    if sudo cryptsetup luksFormat "$LOSP"; then
...
if ! sudo mkfs.ext4 -F "/dev/mapper/${CON_ALIAS}"; then
...
if [[ "${RESP5,,}" = "y" ]]; then
    read -rp "Type the mount path (full path): " MOUNT_PATH
    if [[ -d "${MOUNT_PATH}" ]]; then
        if ! sudo mount "/dev/mapper/${CON_ALIAS}" "${MOUNT_PATH}"; then
...
###################
echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✓ LUKS container successfully created!${NC}"
echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"
