...
6. Be very careful with erasing LUKS container headers. The biggest advantage of removing the header from the container is that it is IMPOSSIBLE to crack, since there is no key to brute-force. This is ideal for users whose require anti-forensics protection. The disadvantage is without a header backup there is **NO DATA RECOVERY**, even if the password is known! Which perhaps could be useful for some users.
