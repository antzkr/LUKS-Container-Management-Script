...
LUKS_HEADER_SIZE=18  # MB for header overhead (LUKS 1 & 2)
SAFETY_BUFFER=${SAFETY_BUFFER:-256} # Safety margin in MB
DATA_GROWTH_BUFFER=${DATA_GROWTH_BUFFER:-512} # Expected data growth in MB
PERCENTAGE_BUFFER=20 # % buffer for filesystem operations
...
calculate_min_safe_size() {
    ...
    echo -e "  Conservative target:       ${GREEN}${min_conservative} MB${NC}"
    echo -e "  Moderate target:           ${YELLOW}${min_moderate} MB${NC}"
    echo -e "  Aggressive target:         ${RED}${min_aggressive} MB${NC}"
    echo -e "  ─────────────────────────────────────────────"
...
backup_luks_header() {
    ...
    echo -e "${YELLOW}Keep backups safe!${NC}\n"
...
restore_luks_header() {
    ...
    if [[ "${check_uuid,,}" == "y" ]]; then
        local temp_loop
        temp_loop=$(sudo losetup -f --show "$backup_file" 2>/dev/null)
        if [[ -n "$temp_loop" ]]; then
            local backup_uuid
...
erase_luks_header() {
    ...
    echo -e "${YELLOW}Only the header area will be overwritten, not the encrypted data.${NC}"
...
while true; do
    ...
    if [[ "$TASK_NUM" == "11" ]]; then
        ...
        if [[ "$TASK_NUM" == "2" ]]; then
            ...
            # Shrink logic guide with minimum size calculation
